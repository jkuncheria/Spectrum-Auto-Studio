...
const Header: React.FC = () => {
  ...
  return (
    <header className="w-full flex flex-col font-sans z-50 relative">
      ...
      {/* Navigation Bar */}
      <nav className={`text-white ${isMenuOpen ? 'block' : 'hidden'} md:block`} style={{ backgroundColor: '#1a1a1a' }}>
        <div className="container mx-auto px-4 md:px-16">
          <ul className="flex flex-col md:flex-row md:justify-center md:space-x-8 lg:space-x-12 text-sm font-extrabold py-4 md:py-0">
            {NAV_ITEMS.map((item) => (
              <li 
                key={item.label} 
                className="border-b md:border-none last:border-none relative" style={{ borderColor: '#333' }}
              >
                {item.subItems && item.subItems.length > 0 ? (
                  ...
                  >
                    ...
                    {openDropdown === item.label && (
                      <ul className="md:absolute md:top-full md:left-0 md:mt-0 md:shadow-lg md:min-w-[200px] md:rounded-b-lg overflow-hidden" style={{ backgroundColor: '#2a2a2a' }}>
