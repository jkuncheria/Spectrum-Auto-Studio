...
const Services: React.FC = () => {
  const services = [
    ...
    },
    {
      ...
      icon: <Circle className="w-8 h-8 text-[#F0E130]" />,
      link: '/wheel-caliper'
    },
    {
      number: '08',
  ...
  return (
    <>
      <SEO
        title="Auto Detailing & Protection Services Phoenix | PPF, Ceramic, Tint, Wraps"
        description="Complete auto protection services in Phoenix. Paint protection film, ceramic coatings, window tinting, vehicle wraps, detailing, and paint correction. Free quotes."
        keywords="auto detailing Phoenix, PPF Phoenix, ceramic coating Phoenix, window tinting Phoenix, vehicle wraps Phoenix, paint correction Phoenix"
      ...
      <section className="py-24 bg-stone-950 px-4 md:px-16">
        ...
        <div className="text-center mb-20">
          <span className="text-[#F0E130] font-bold tracking-wider text-sm uppercase">OUR SERVICES</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          {services.map((service, idx) => (
            <div 
              key={idx}
              className="bg-stone-900 rounded-2xl p-8 md:p-10 hover:bg-stone-800 transition-all duration-300 border border-stone-800 hover:border-[#F0E130] hover:shadow-xl"
            >
              <div className="flex items-start gap-6">
                <div className="flex-shrink-0">
                  <div className="bg-[#F0E130] text-white text-2xl font-black w-16 h-16 rounded-xl flex items-center justify-center">
                    {service.number}
                  </div>
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="bg-stone-800 p-3 rounded-lg shadow-sm">
                      {service.icon}
                    </div>
                  ...
                  <p className="text-[#F0E130] font-semibold text-sm mb-4">
                    {service.cta}
                  </p>
                  <Link 
                    to={service.link || '/contact'}
                    className="inline-flex items-center gap-2 text-[#F0E130] font-bold hover:text-yellow-400 transition-colors group"
                  >
                    Learn More
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
          {mainServices.map((service, idx) => (
            <div 
              key={idx}
              className="bg-gradient-to-br from-stone-900 to-stone-800 rounded-2xl p-8 md:p-10 text-white"
            >
              ...
              <Link 
                to={service.link}
                className="inline-flex items-center gap-2 bg-white text-[#F0E130] hover:bg-gray-100 font-bold py-4 px-8 rounded-xl transition-all transform hover:scale-105 shadow-lg group"
              >
                {service.cta}
        ...
        <div className="bg-gradient-to-br from-stone-900 to-stone-800 rounded-2xl p-12 md:p-16 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link 
              to="/contact" 
              className="bg-[#F0E130] hover:bg-yellow-400 text-black font-bold py-4 px-8 rounded-xl transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-2 group"
            >
              Get Free Quote
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Link>
            <a 
              href="tel:6025542113" 
              className="border-2 border-[#F0E130] text-[#F0E130] hover:bg-[#F0E130] hover:text-black font-bold py-4 px-8 rounded-xl transition-all flex items-center justify-center gap-2"
            >
